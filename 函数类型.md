...
上面示例中，参数 `x` 的类型是 `number|undefined`，表示要么传入一个值，要么传入 `undefined`，如果省略这个参数，就会报错。
...
上面示例中，参数 `arr` 的类型是 `readonly number[]`，表示为只读参数。入股哦函数体内部修改这个数组，就会报错。
...
上面示例中，函数 `f()` 的返回值类型是 `void`，但是实际上返回了一个数值，编译时就报错了。
...
需要特别注意的是，如果有变量、对象方法、函数参数从一个返回值为 void 类型的参数，那么并不代表不能赋值为有返回值的函数。恰恰相反，该变量、对象方法和函数参数可以接受返回任意值的函数，这时并不会报错。
...
注意，这种情况仅限于变量、对象方法和函数参数，函数字面量如果声明了返回值是 void 类型，还是不能有返回值的。
