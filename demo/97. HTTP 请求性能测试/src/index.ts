...
function parseArgs(args: string[]): BenchConfig {
    ...
    while (i < args.length) {
        ...
        switch (arg) {
            ...
            case "--requests":
                i++;
                if (i < args.length) config.totalRequests = parseInt(args[i]!, 10);
                break;
            case "-c":
            case "--concurrency":
                i++;
                if (i < args.length) config.concurrency = parseInt(args[i]!, 10);
                break;
            case "-m":
            case "--method":
                i++;
                if (i < args.length) config.method = args[i]!.toUpperCase();
                break;
            case "-H":
            case "--header": {
                i++;
            ...
            case "--data":
                i++;
                if (i < args.length) config.body = args[i]!;
                break;
            case "-t":
            case "--timeout":
                i++;
                if (i < args.length) config.timeout = parseInt(args[i]!, 10);
                break;
            case "--json":
...
/** 发送单次 HTTP 请求并返回结果 */
function sendRequest(config: BenchConfig): Promise<RequestResult> {
    return new Promise((resolve) => {
        const startTime = performance.now();
        let bytesReceived = 0;
        let resolved = false;

        const done = (result: Omit<RequestResult, "timestamp">) => {
            if (resolved) return;
            resolved = true;
            resolve({
                ...result,
...
class HttpBenchmarker {
    ...
    private printBanner(): void {
        ...
        console.log(
            COLOR.bold(
                COLOR.cyan("  ║     HTTP 请求性能测试工具 v1.0.0        ║"),
            ),
        );
        console.log(
            COLOR.bold(
    ...
    private printResults(): void {
        ...
        console.log(
            `  ${COLOR.bold("错误率:")}       ${stats.failedRequests > 0 ? COLOR.red(`${((stats.failedRequests / stats.totalRequests) * 100).toFixed(2)}%`) : COLOR.green("0.00%")}`,
        );
        console.log("");
        ...
        console.log(
            `  ${COLOR.bold("吞吐量:")}       ${COLOR.green(stats.requestsPerSec.toFixed(2))} req/s`,
        );
        console.log(
            `  ${COLOR.bold("总耗时:")}       ${formatDuration(stats.totalDuration)} (${stats.totalDuration.toFixed(0)} ms)`,
        );
        console.log(
            `  ${COLOR.bold("数据传输:")}     ${formatBytes(stats.totalBytes)}`,
        );
        console.log(
            `  ${COLOR.bold("平均吞吐:")}     ${formatBytes(stats.totalBytes / (stats.totalDuration / 1000))}/s`,
        );
        console.log("");
        ...
        if (stats.statusCodes.size > 0) {
            ...
            const sortedCodes = [...stats.statusCodes.entries()].sort(
                ([a], [b]) => a - b,
            );
            for (const [code, count] of sortedCodes) {
                ...
                console.log(
                    `  ${colorFn(String(code))}  ${count.toString().padStart(6)} 次  (${percent}%)`,
                );
            }
            console.log("");
        }
    ...
    private printLatencyHistogram(stats: BenchStats): void {
        ...
        for (const range of ranges) {
            const count = stats.latencies.filter(
            ...
            const maxCount = Math.max(
                ...ranges.map((r) =>
                    stats.latencies.filter((l) => l >= r.min && l < r.max).length
                ),
            );
            ...
            console.log(
                `  ${COLOR.gray(label)} ${COLOR.cyan(bar)} ${count} (${percent}%)`,
            );
        }
    ...
    /** 构建直方图区间 */
    private buildHistogramRanges(stats: BenchStats): Array<{ min: number; max: number; label: string }> {
        const ranges: Array<{ min: number; max: number; label: string }> = [];
        const max = stats.maxLatency;

        // 根据最大延迟动态选择区间大小
        let bucketSize: number;
        if (max <= 50) bucketSize = 5;
        else if (max <= 200) bucketSize = 20;
        else if (max <= 500) bucketSize = 50;
        else if (max <= 2000) bucketSize = 200;
        else if (max <= 10000) bucketSize = 1000;
        else bucketSize = 5000;
