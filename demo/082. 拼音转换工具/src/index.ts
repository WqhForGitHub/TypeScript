...
const TONE_DIGITS = [1, 2, 3, 4, 5] as const;
type ToneDigit = (typeof TONE_DIGITS)[number];

/** 形如 "hao3" 的拼音音节（字母串 + 声调数字） */
type PinyinSyllable = `${string}${ToneDigit}`;
...
interface ConvertAmbiguous {
  readonly kind: "ambiguous";
  readonly input: string;
  readonly candidates: readonly string[];
}

type ConvertResult = ConvertSuccess | ConvertError | ConvertAmbiguous;
...
interface ConverterConfig {
  readonly defaultTone: ToneStyle;
...
function numberToMark(py: string): string {
  ...
  else if (iuIdx >= 0) targetIdx = iuIdx + 1;
  else if (uiIdx >= 0) targetIdx = uiIdx;
  ...
  const replacement =
    vowelChar === vowelChar.toUpperCase() ? marked.toUpperCase() : marked;
  body =
    body.substring(0, targetIdx) + replacement + body.substring(targetIdx + 1);
...
function buildEntry(
  ...
  const entry: PinyinEntry = {
    number: numberForm,
    mark: numberToMark(numberForm),
    initial: initialChar,
  };
  if (source !== undefined) {
    const mutable: Mutable<PinyinEntry> = { ...entry };
...
/** 根据声调风格格式化拼音条目（重载） */
function formatPinyin(entry: PinyinEntry, style: ToneStyle.Mark): string;
function formatPinyin(entry: PinyinEntry, style: ToneStyle.Number): string;
function formatPinyin(entry: PinyinEntry, style: ToneStyle.None): string;
function formatPinyin(entry: PinyinEntry, style: ToneStyle): string;
function formatPinyin(entry: PinyinEntry, style: ToneStyle): string {
  if (style === ToneStyle.Mark) return entry.mark;
  if (style === ToneStyle.Number) return entry.number;
  return entry.number.replace(/[1-5]$/, "");
}

// ## 11. 类型守卫

function isConvertSuccess(r: ConvertResult): r is ConvertSuccess {
  return r.kind === "success";
}
function isConvertError(r: ConvertResult): r is ConvertError {
  return r.kind === "error";
}
function isPinyinError(e: unknown): e is PinyinError {
  return e instanceof PinyinError;
}
/** 类型守卫：判断字符串是否为合法拼音音节 */
function isPinyinSyllable(s: string): s is PinyinSyllable {
  return /^[a-z]+[1-5]$/i.test(s);
}
/** 类型守卫：CharInfo 是否为带拼音的汉字 */
function isResolvedHanzi(
  info: CharInfo,
): info is CharInfo & { entry: PinyinEntry; type: typeof CharType.Hanzi } {
  return info.type === CharType.Hanzi && info.entry !== undefined;
}

// ## 12. 泛型存储类（带约束 + 迭代器 + 生成器 + getter/setter + Symbol）

class PinyinStore<T extends Identifiable> implements Iterable<T> {
  private readonly items = new Map<string, T>();
  readonly [STORE_ID]: string;
  private [INTERNAL_VERSION]: number;
...
/** 词典条目：id 即汉字字符 */
interface DictItem extends Identifiable {
  readonly char: string;
  readonly entry: PinyinEntry;
}
...
abstract class AbstractPinyinConverter {
  protected readonly store: PinyinStore<DictItem>;
  protected _toneStyle: ToneStyle;

  constructor(
    store: PinyinStore<DictItem>,
    toneStyle: ToneStyle = DEFAULT_CONFIG.defaultTone,
  ) {
    this.store = store;
    this._toneStyle = toneStyle;
  }

  get toneStyle(): ToneStyle {
    return this._toneStyle;
  }

  set toneStyle(value: ToneStyle) {
    const valid = (Object.values(ToneStyle) as string[]).includes(
      value as string,
    );
    if (!valid) {
  ...
  abstract convert(text: string): ConvertResult;
  abstract classify(ch: string): CharType;

  protected lookup(ch: string): PinyinEntry | undefined {
...
class FullPinyinConverter extends AbstractPinyinConverter {
  ...
  classify(ch: string): CharType {
    if (this.store.has(ch)) return CharType.Hanzi;
    if (/\s/.test(ch)) return CharType.Space;
    if (/[，。！？、；：""''（）【】,.!?;:]/.test(ch))
  ...
  /** 生成器：逐字分类 */
  *iterate(text: string): IterableIterator<CharInfo> {
...
function loadUserDict(userPath: string): Record<string, string> {
  ...
  try {
    const raw = JSON.parse(fs.readFileSync(userPath, "utf-8")) as unknown;
...
function buildStore(userDict: Record<string, string>): PinyinStore<DictItem> {
  const store = new PinyinStore<DictItem>("pinyin-main");
...
function parseTone(v: string | undefined): ToneStyle {
  if (v === ToneStyle.Number) return ToneStyle.Number;
  if (v === ToneStyle.None) return ToneStyle.None;
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let char = "";
  let pinyin = "";
  let tone: ToneStyle = ToneStyle.Mark;
...
function describeResult(result: ConvertResult): string {
  ...
  switch (opts.command) {
    ...
    case "test": {
      ...
      try {
        ...
        bad.toneStyle = "wrong" as unknown as ToneStyle;
      } catch (e) {
        if (isPinyinError(e)) {
