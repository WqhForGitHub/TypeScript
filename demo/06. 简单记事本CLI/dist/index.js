...
function findNoteIndex(store, id) {
...
function cmdList() {
    ...
    for (const note of store.notes) {
        console.log(`${note.id}\t${formatDate(note.createdAt)}\t${formatDate(note.updatedAt)}\t${note.title}`);
    }
    console.log("--------------------------------------------------");
}
...
function cmdSearch(keyword) {
    ...
    const store = loadStore();
    const hits = store.notes.filter((n) => n.title.toLowerCase().includes(kw) ||
    ...
    for (const note of hits) {
        console.log(`ID ${note.id} | ${formatDate(note.updatedAt)} | ${note.title}`);
        // 输出第一行作为预览
