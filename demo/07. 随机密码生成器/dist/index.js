...
function generatePassword(options) {
    ...
    if (requiredChars.length > options.length) {
        throw new Error(`密码长度 ${options.length} 不足以包含所有已启用字符类型（至少需要 ${requiredChars.length} 位）`);
    }
    // 填充剩余长度
 ...
 * 过滤掉被排除的字符
 */
function filterExcluded(charset, exclude) {
    if (!exclude)
        return charset;
...
function evaluateStrength(password) {
    ...
    score += uniformScore;
    details.push(`字符唯一率 ${(uniqueRatio * 100).toFixed(0)}% (+${uniformScore}分)`);
    // 确定等级（分数上限 100）
...
function main() {
    ...
    try {
        ...
        if (options.symbols)
            charsetInfo.push("符号");
        console.log(`配置: ${options.length} 位 | 字符集: ${charsetInfo.join("+")}${options.exclude ? ` | 排除: "${options.exclude}"` : ""}\n`);
    }
    catch (error) {
