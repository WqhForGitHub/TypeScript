...
(function (TokenType) {
    ...
    TokenType["Mixed"] = "mixed";
})(TokenType || (TokenType = {}));
var ErrorCode;
(function (ErrorCode) {
    ErrorCode["EmptyInput"] = "EMPTY_INPUT";
    ErrorCode["InvalidMode"] = "INVALID_MODE";
    ErrorCode["InvalidN"] = "INVALID_N";
    ErrorCode["NotFound"] = "NOT_FOUND";
    ErrorCode["Unknown"] = "UNKNOWN";
})(ErrorCode || (ErrorCode = {}));
var SegmentMode;
(function (SegmentMode) {
    SegmentMode["Whitespace"] = "whitespace";
    SegmentMode["Char"] = "char";
    SegmentMode["Dict"] = "dict";
    SegmentMode["Ngram"] = "ngram";
...
// ---- Symbol 唯一属性键 ----
const tokenIdSym = Symbol("tokenId");
...
// ---- 类型守卫 (Type guards) ----
function isTokenSuccess(r) {
    return r.success === true && r.count > 0;
}
function isTokenError(r) {
    return r.success === false;
}
function isTokenEmpty(r) {
    return r.success === true && r.count === 0;
}
function isSegmentMode(v) {
...
function countFrequency(tokens, topN) {
    const map = new Map();
...
function extractKeywords(text, count) {
    const tokens = segmentChinese(text, BUILTIN_DICT).filter((t) => !STOP_WORDS.has(t.toLowerCase()) && t.length > 1);
    return countFrequency(tokens, count);
}
function tokenize(text, mode = SegmentMode.Whitespace, n = DEFAULTS.n) {
    if (text.length === 0)
...
function safeTokenize(text, mode, n) {
    try {
        if (text.length === 0) {
            return { success: true, tokens: [], count: 0 };
        }
        ...
        const msg = e instanceof TokenizeError ? e.message : e.message;
...
function createTokenizer(lang, dict = BUILTIN_DICT) {
    switch (lang) {
...
function createTokenRecord(text, position) {
    const meta = {
...
function buildStoreFromText(text, mode) {
...
function parseArgs(argv) {
    ...
    let text = "";
    let mode = SegmentMode.Whitespace;
    let n = DEFAULTS.n;
...
function runCommand(opts) {
    switch (opts.command) {
        ...
        case "segment": {
            ...
            console.log(`中文分词结果 (共 ${tokens.length} 个词, 语言=${tok.language}):`);
        ...
        case "store": {
            ...
            for (const rec of store) {
                console.log(`  [${rec.meta.position}] "${rec.text}" type=${rec.meta.type} lang=${rec.meta.language} id=${rec.id}`);
            }
            break;
        }
        case "safe": {
