...
interface ProxyConfig {
    ...
    blocklist: Set<string>; // 域名黑名单
    allowlist: Set<string>; // 域名白名单（为空则不过滤）
    allowlistMode: boolean; // 是否启用白名单模式
    adminPath: string; // 管理面板路径
}
...
function handleHttpRequest(
    ...
    if (isBlocked(hostname)) {
        ...
        console.log(
            `[${now()}] 拦截: ${req.method} ${requestUrl} (域名已被屏蔽)`,
        );
    ...
    if (cached) {
        ...
        console.log(
            `[${now()}] 缓存命中: ${req.method} ${requestUrl} (${duration}ms)`,
        );
    ...
    const proxyReq = http.request(options, (proxyRes) => {
        ...
        proxyRes.on("end", () => {
            ...
            console.log(
                `[${now()}] 代理: ${req.method} ${requestUrl} → ${proxyRes.statusCode} (${duration}ms)`,
            );
...
function main(): void {
    ...
    server.listen(config.port, config.host, () => {
        ...
        console.log(
            `  白名单模式: ${config.allowlistMode ? "已启用" : "已禁用"}`,
        );
