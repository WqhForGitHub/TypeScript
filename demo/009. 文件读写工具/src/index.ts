...
function getFileType(stats: fs.Stats): string {
  if (stats.isFile()) return "普通文件";
  if (stats.isDirectory()) return "目录";
  if (stats.isSymbolicLink()) return "符号链接";
  if (stats.isBlockDevice()) return "块设备";
  if (stats.isCharacterDevice()) return "字符设备";
  if (stats.isFIFO()) return "FIFO (命名管道)";
...
/** 安全检查：确认路径存在 */
function assertPathExists(filePath: string, label?: string): string {
  const resolved: string = resolvePath(filePath);
...
function assertPathNotExists(filePath: string): string {
  const resolved: string = resolvePath(filePath);
  if (fs.existsSync(resolved)) {
...
function cmdRead(filePath: string, options: string[]): void {
  const resolved: string = assertPathExists(filePath, "文件");
  const stat: fs.Stats = fs.statSync(resolved);

  if (stat.isDirectory()) {
    console.error(
      `错误：${resolved} 是一个目录，不是文件。请使用 list 命令查看目录。`,
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < options.length; i++) {
    if (options[i] === "-l" && options[i + 1]) {
      ...
      encoding = options[i + 1]! as BufferEncoding;
      i++;
    }
  }

  try {
    const content: string = fs.readFileSync(resolved, encoding);
...
function cmdWrite(filePath: string, content: string, options: string[]): void {
  const resolved: string = resolvePath(filePath);
  const addNewline: boolean = !options.includes("-n");

  // 确保父目录存在
  const dir: string = path.dirname(resolved);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
    console.log(`已创建目录: ${dir}`);
  }

  const finalContent: string = addNewline ? content + "\n" : content;
  ...
  try {
    ...
    console.error(`写入文件失败：${(err as Error).message}`);
    process.exit(1);
  }
}
...
function cmdAppend(filePath: string, content: string): void {
  ...
  const existed: boolean = fs.existsSync(resolved);
  ...
  try {
    ...
    console.error(`追加文件失败：${(err as Error).message}`);
    process.exit(1);
  }
}
 ...
 *   -f           强制覆盖目标文件
 */
function cmdCopy(srcPath: string, destPath: string, options: string[]): void {
  const srcResolved: string = assertPathExists(srcPath, "源");
  const destResolved: string = resolvePath(destPath);
  const force: boolean = options.includes("-f");

  // 检查源路径是否为文件
  const srcStat: fs.Stats = fs.statSync(srcResolved);
  ...
  try {
    // 确保目标父目录存在
    const destDir: string = path.dirname(destResolved);
    ...
    console.error(`复制文件失败：${(err as Error).message}`);
    process.exit(1);
  }
}
 ...
 *   -f           强制覆盖目标文件
 */
function cmdMove(srcPath: string, destPath: string, options: string[]): void {
  const srcResolved: string = assertPathExists(srcPath, "源");
  const destResolved: string = resolvePath(destPath);
  const force: boolean = options.includes("-f");
  ...
  try {
    ...
    console.error(`移动文件失败：${(err as Error).message}`);
    process.exit(1);
  }
}
...
function cmdDelete(filePath: string, options: string[]): void {
  const resolved: string = assertPathExists(filePath);
  const force: boolean = options.includes("-f");
  ...
  try {
    ...
    console.error(`删除文件失败：${(err as Error).message}`);
    process.exit(1);
  }
}
...
function cmdInfo(filePath: string): void {
  ...
  console.log(
    `大小:       ${formatSize(stat.size)} (${stat.size.toLocaleString()} 字节)`,
  );
  ...
  if (stat.isFile()) {
    const ext: string = path.extname(resolved);
    const name: string = path.basename(resolved);
    console.log(`文件名:     ${name}`);
    console.log(`扩展名:     ${ext || "(无)"}`);
  } else if (stat.isDirectory()) {
    try {
      const entries: string[] = fs.readdirSync(resolved);
      const fileCount: number = entries.filter((e) =>
        fs.statSync(path.join(resolved, e)).isFile(),
      ).length;
      const dirCount: number = entries.filter((e) =>
...
function cmdList(dirPath: string, options: string[]): void {
  const resolved: string = assertPathExists(dirPath, "目录");
  const stat: fs.Stats = fs.statSync(resolved);
  ...
  const showHidden: boolean = options.includes("-a");
  const longFormat: boolean = options.includes("-l");

  try {
    let entries: string[] = fs.readdirSync(resolved);
    ...
    if (longFormat) {
      ...
      for (const entry of entries) {
        const fullPath: string = path.join(resolved, entry);
        try {
          const entryStat: fs.Stats = fs.statSync(fullPath);
          const isDir: boolean = entryStat.isDirectory();
          const type: string = isDir ? "[D]" : getFileIcon(entry, false);
          const size: string = isDir ? "-" : formatSize(entryStat.size);
          const time: string = formatDate(entryStat.mtime);

          console.log(
            type.padEnd(6) + size.padEnd(12) + time.padEnd(22) + entry,
          );
        } catch {
          console.log(
            "[?]".padEnd(6) + "-".padEnd(12) + "-".padEnd(22) + entry,
          );
        }
      }
    } else {
    ...
    console.error(`读取目录失败：${(err as Error).message}`);
    process.exit(1);
  }
}
...
function cmdExists(filePath: string): void {
  const resolved: string = resolvePath(filePath);
  const exists: boolean = fs.existsSync(resolved);

  if (exists) {
    const stat: fs.Stats = fs.statSync(resolved);
    const type: string = stat.isDirectory() ? "目录" : "文件";
 ...
 *   -p           递归创建（默认行为）
 */
function cmdMkdir(dirPath: string, _options: string[]): void {
  const resolved: string = resolvePath(dirPath);
  ...
  try {
    ...
    console.error(`创建目录失败：${(err as Error).message}`);
    process.exit(1);
  }
}
...
function cmdTree(dirPath: string, options: string[]): void {
  ...
  function printTree(currentPath: string, prefix: string, depth: number): void {
    ...
    entries.forEach((entry, index) => {
      const fullPath: string = path.join(currentPath, entry);
      ...
      try {
        ...
        if (entryStat.isDirectory()) {
          ...
          fileCount++;
          console.log(
            `${prefix}${connector}${entry}  (${formatSize(entryStat.size)})`,
          );
        }
      } catch {
...
function parseArgs(argv: string[]): {
  ...
  const command: string = (positional[0] ?? "help").toLowerCase();
  // positional[0] 是命令，之后的是位置参数
  const args: string[] = positional.slice(1);
  ...
  switch (command) {
    ...
    case "write":
      if (!positional[0] || !positional[1]) {
        console.error(
          "错误：请提供文件路径和内容。用法：file-rw write <文件路径> <内容>",
        );
        process.exit(1);
      }
    ...
    case "append":
      if (!positional[0] || !positional[1]) {
        console.error(
          "错误：请提供文件路径和内容。用法：file-rw append <文件路径> <内容>",
        );
        process.exit(1);
      }
    ...
    case "cp":
      if (!positional[0] || !positional[1]) {
        console.error(
          "错误：请提供源路径和目标路径。用法：file-rw copy <源路径> <目标路径>",
        );
        process.exit(1);
      }
    ...
    case "rename":
      if (!positional[0] || !positional[1]) {
        console.error(
          "错误：请提供源路径和目标路径。用法：file-rw move <源路径> <目标路径>",
        );
        process.exit(1);
      }
