...
/** 向单个客户端发送消息 */
function sendTo(client: Client, message: string): void {
...
/** 向所有客户端广播消息 */
function broadcast(message: string, excludeId?: number): void {
...
/** 向房间内所有成员广播消息 */
function broadcastToRoom(roomName: string, message: string, excludeId?: number): void {
...
/** 处理 /nick 命令 */
function handleNick(client: Client, args: string[]): void {
  ...
  const exists = Array.from(clients.values()).some(
    (c) => c.nickname === newNick && c.id !== client.id
  );
  if (exists) {
...
/** 处理 /msg 命令（私聊） */
function handleMsg(client: Client, args: string[]): void {
  if (args.length < 2) {
...
/** 处理 /join 命令（加入房间） */
function handleJoin(client: Client, args: string[]): void {
...
/** 处理 /leave 命令（离开房间） */
function handleLeave(client: Client, args: string[]): void {
  if (!client.room) {
...
/** 处理 /room 命令（房间内聊天） */
function handleRoomMsg(client: Client, args: string[]): void {
  if (!client.room) {
  ...
  broadcastToRoom(
    client.room,
    `[${now()}] [${client.room}] ${client.nickname}: ${message}`,
    client.id
  );
...
/** 处理 /rooms 命令 */
function handleRooms(client: Client): void {
  if (rooms.size === 0) {
...
/** 处理 /users 命令 */
function handleUsers(client: Client): void {
...
/** 处理 /help 命令 */
function handleHelp(client: Client): void {
  const helpText = [
    `[${now()}] 系统: 可用命令:`,
    "  /nick <名称>        - 修改昵称",
    "  /msg <昵称> <消息>   - 发送私聊",
