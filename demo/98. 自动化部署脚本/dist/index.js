...
async function main() {
    ...
    const log = new logger_1.Logger(args.verbose);
    // ─── 初始化配置文件 ──────────────────────────────────────
    if (args.init) {
        ...
        (0, config_1.generateDefaultConfig)(configPath);
    ...
    let config;
    if (args.config) {
        const loaded = (0, config_1.loadConfigFromFile)(args.config);
        if (!loaded) {
        ...
        config = (0, config_1.buildConfigFromArgs)(args);
    ...
    // ─── 显示配置摘要 ───────────────────────────────────────
    log.separator();
    (0, config_1.printConfigSummary)(config, log);
    log.separator();
    // ─── 确认部署 ────────────────────────────────────────────
    if (!args.dryRun) {
    ...
    (0, tasks_1.setDemoMode)(!realMode);
    if (!realMode && !args.dryRun) {
        log.info("当前为 Demo 模式 (模拟执行)，设置 DEPLOY_REAL=1 启用真实执行");
    }
    // ─── 创建并运行流水线 ───────────────────────────────────
    log.resetTimer();
    const pipeline = (0, pipeline_1.createDefaultPipeline)(config, log);
    const result = await pipeline.run(args.dryRun);
    // ─── 输出部署摘要 ───────────────────────────────────────
    log.summary({
