...
// ─── HMR 运行时 ──────────────────────────────────────────
class HMR extends events_1.EventEmitter {
    constructor(options) {
        super();
        /** 已加载的模块注册表 */
    ...
    import(modulePath) {
        ...
        if (this.modules.has(resolved)) {
            return this.modules.get(resolved).proxy;
        }
     ...
     */
    getModuleIds() {
        return Array.from(this.modules.keys()).map(id => path.relative(this.rootDir, id));
    }
    // ─── 模块加载 ────────────────────────────────────────
    /**
     * 加载模块：读取源码 → 编译 → 执行 → 注册
    ...
    loadModule(resolvedPath) {
        ...
        let disposeCallback;
        // 执行编译后的代码，填充 exports
        const finalExports = this.executeModule(compiledSource, resolvedPath, moduleExports, state, {
            onAccept: (cb) => { acceptCallback = cb; },
            onDispose: (cb) => { disposeCallback = cb; },
        });
    ...
    compile(source, filePath) {
        ...
        if (result.diagnostics && result.diagnostics.length > 0) {
            const errors = result.diagnostics.map(d => {
                ...
                const line = d.file && d.start
                    ? d.file.getLineAndCharacterOfPosition(d.start).line + 1
                    : '?';
                return `  行 ${line}: ${msg}`;
            });
            throw new Error(`TypeScript 编译错误 (${path.relative(this.rootDir, filePath)}):\n${errors.join('\n')}`);
        }
     ...
     * @returns 模块的最终导出对象
     */
    executeModule(compiledSource, filePath, moduleExports, hmrState, hmrCallbacks) {
        ...
        try {
            ...
            const customRequire = (id) => {
                if (id.startsWith('.')) {
                    const resolved = this.resolveModule(id, path.dirname(filePath));
            ...
            console.error(`[HMR] 模块执行错误 ${path.relative(this.rootDir, filePath)}:`, err);
            throw err;
        }
    }
    // ─── Proxy 代理 ─────────────────────────────────────
    ...
    scheduleReload(filePath) {
        ...
        this.debounceTimers.set(resolvedPath, setTimeout(() => {
    ...
    handleFileChange(resolvedPath) {
        ...
        try {
            // 检查文件是否确实变化
            const stat = fs.statSync(resolvedPath);
            if (stat.mtime <= entry.lastModified)
                return;
            ...
            // 3. 执行新代码
            const newExports = {};
            let newAcceptCallback;
            let newDisposeCallback;
            this.executeModule(newCompiled, resolvedPath, newExports, entry.state, {
                onAccept: (cb) => { newAcceptCallback = cb; },
                onDispose: (cb) => { newDisposeCallback = cb; },
            });
     ...
     */
    resolveModule(modulePath, fromDir) {
