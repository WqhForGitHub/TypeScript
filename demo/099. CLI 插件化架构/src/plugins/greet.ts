...
import { isUserLoginEvent } from "./logger";
...
type GreetEvent =
  | {
      ...
      readonly style: GreetStyle;
    }
  | { readonly kind: "bye"; readonly name: string };

/** 类型守卫: 判断事件是否为 greet */
function isGreetEvent(
  ev: GreetEvent,
): ev is Extract<GreetEvent, { readonly kind: "greet" }> {
  return ev.kind === "greet";
}

/** 问候计数记录 */
interface GreetCounters {
  greet: number;
  bye: number;
}
...
/** 别名列表 (as const, 仅字面量) */
const GREET_ALIASES = ["hello", "hi"] as const;
const BYE_ALIASES = ["goodbye", "farewell"] as const;
...
class GreetPlugin extends BasePlugin {
  /** 问候计数器 (私有) */
  private readonly counters: GreetCounters = { greet: 0, bye: 0 };
  ...
  /** 问候次数 (getter) */
  public get greetCount(): number {
    return this.counters.greet;
  }

  /** 告别次数 (getter) */
  public get byeCount(): number {
  ...
  /** 生成问候语 (指定风格, 重载) */
  private greet(name: string, style: GreetStyle): string;
  /** greet 实现 */
  private greet(name: string, style: GreetStyle = GreetStyle.Casual): string {
  ...
  protected onInit(_context: PluginContext): void {
    ...
    const greetCommand = {
      ...
      handler: (args: CommandArgs): void => {
        ...
        const style: GreetStyle =
          args.options.formal === true ? GreetStyle.Formal : GreetStyle.Casual;
        ...
        // 发出问候事件
        this.emit("greet:occurred", { kind: "greet" as const, name, style });
      },
    } satisfies Command;
    ...
    const byeCommand = {
      ...
      handler: (args: CommandArgs): void => {
        ...
        this.emit("greet:occurred", { kind: "bye" as const, name });
      },
    } satisfies Command;
    ...
    this.on("user:login", (data: unknown) => {
      if (!isUserLoginEvent(data)) return;
      console.log(`\x1b[32m欢迎回来, ${data.name}!\x1b[0m`);
    });

    // 监听自身的问候事件 (使用判别联合 + 类型守卫)
    this.on("greet:occurred", (data: unknown) => {
      if (!isGreetEventRecord(data)) return;
...
function isGreetEventRecord(data: unknown): data is GreetEvent {
