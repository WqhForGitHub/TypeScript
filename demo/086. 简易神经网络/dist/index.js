...
(function (ErrorCode) {
    ...
    ErrorCode["MaxEpochs"] = "MAX_EPOCHS";
})(ErrorCode || (ErrorCode = {}));
var LayerType;
(function (LayerType) {
...
(function (NetworkState) {
    NetworkState["Untrained"] = "UNTRAINED";
    NetworkState["Training"] = "TRAINING";
    NetworkState["Trained"] = "TRAINED";
    NetworkState["Converged"] = "CONVERGED";
})(NetworkState || (NetworkState = {}));
var TrainingPhase;
(function (TrainingPhase) {
    TrainingPhase["Forward"] = "FORWARD";
    TrainingPhase["Backward"] = "BACKWARD";
    TrainingPhase["Update"] = "UPDATE";
    TrainingPhase["EpochEnd"] = "EPOCH_END";
})(TrainingPhase || (TrainingPhase = {}));
// ====================== 自定义错误层次 ======================
class NNError extends Error {
    constructor(code, message) { super(message); this.name = "NNError"; this.code = code; }
}
class DimensionError extends NNError {
    constructor(message) { super(ErrorCode.DimMismatch, message); this.name = "DimensionError"; }
}
class ParseError extends NNError {
    constructor(message) { super(ErrorCode.ParseError, message); this.name = "ParseError"; }
}
function isTrainSuccess(r) { return r.kind === "success"; }
function isTrainError(r) { return r.kind === "error"; }
function isTrainConverged(r) { return r.kind === "converged"; }
function withLearningRate(config, lr) {
    const m = { ...config, hidden: [...config.hidden] };
    m.learningRate = lr;
    return m;
}
// ====================== Symbol 唯一键 ======================
...
class Matrix {
    ...
    assertSameShape(b) {
        if (this.rows !== b.rows || this.cols !== b.cols)
            throw new DimensionError(`矩阵形状不一致 ${this.rows}x${this.cols} vs ${b.rows}x${b.cols}`);
    }
    add(b) {
    ...
    mul(b) {
        if (this.cols !== b.rows)
            throw new DimensionError(`矩阵维度不匹配 ${this.rows}x${this.cols} * ${b.rows}x${b.cols}`);
...
class AbstractActivation {
    toString() { return `Activation(${this.type})`; }
}
class SigmoidActivation extends AbstractActivation {
    ...
    forward(x) { return x.map((v) => 1 / (1 + Math.exp(-v))); }
    derivative(a) { return a.map((v) => v * (1 - v)); }
}
class TanhActivation extends AbstractActivation {
    ...
    forward(x) { return x.map((v) => Math.tanh(v)); }
    derivative(a) { return a.map((v) => 1 - v * v); }
}
class ReluActivation extends AbstractActivation {
    ...
    forward(x) { return x.map((v) => (v > 0 ? v : 0)); }
    derivative(a) { return a.map((v) => (v > 0 ? 1 : 0)); }
}
...
function makeActivation(t) {
    ...
    if (!f)
        throw new NNError(ErrorCode.InvalidInput, `未知激活类型: ${t}`);
    return f();
}
// ====================== 泛型全连接层 ======================
class Layer {
    constructor(neuronCount, layerType, weights, activation, index) {
    ...
    get inputSize() { return this.weights.rows - 1; }
    get weightCols() { return this.weights.cols; }
    get activationType() { return this.activation.type; }
    setWeights(w) {
        if (w.rows !== this.weights.rows || w.cols !== this.weights.cols)
            throw new DimensionError(`权重形状不匹配 ${w.rows}x${w.cols}`);
        this.weights = w;
    }
    setActivation(t) { this.activation = makeActivation(t); }
    snapshot() {
        return { weights: Array.from(this.weights.data), rows: this.weights.rows, cols: this.weights.cols, activation: this.activation.type, layerType: this.layerType };
    }
    /** 生成器：逐神经元迭代（每神经元的权重列） */
    *neurons() {
...
// ====================== 损失函数 / 工具 ======================
function computeLoss(name, pred, target) {
    let sum = 0;
    for (let i = 0; i < pred.data.length; i++) {
        ...
        if (name === "mse")
            ...
            sum += -(t * Math.log(p + 1e-12) + (1 - t) * Math.log(1 - p + 1e-12));
    }
...
class NeuralNetwork {
    constructor(config) {
        this.layers = [];
        this._state = NetworkState.Untrained;
        this._epoch = 0;
        this[_a] = [];
        this._config = config;
    ...
    build() {
        const sizes = [this._config.inputSize, ...this._config.hidden, this._config.outputSize];
        for (let i = 0; i < sizes.length - 1; i++) {
            ...
            const actType = isOutput ? this._config.outputActivation : this._config.hiddenActivation;
            const lt = i === 0 ? LayerType.Input : (isOutput ? LayerType.Output : LayerType.Hidden);
            this.layers.push(new Layer(outSize, lt, w, makeActivation(actType), i));
        }
    }
    get config() { return this._config; }
    get state() { return this._state; }
    get epoch() { return this._epoch; }
    get learningRate() { return this._learningRate; }
    set learningRate(v) {
        if (v <= 0)
            throw new NNError(ErrorCode.InvalidInput, "学习率必须为正数");
    ...
    trainBatch(x, y) {
        ...
        if (last.activationType === ActivationType.Softmax && this._config.loss === "crossEntropy") {
            delta = pred.sub(y); // softmax + 交叉熵的简化梯度
        }
        else {
            const dAct = last.activation.derivative(last.lastA);
            const dLoss = this._config.loss === "mse" ? pred.sub(y).scale(2 / y.cols) : pred.sub(y);
        ...
        for (let l = this.layers.length - 1; l >= 0; l--) {
            ...
            const grad = input.transpose().mul(delta).scale(this._learningRate / x.rows);
    ...
    fit(inputs, targets, epochs, batchSize, verbose = true, convergeThreshold = 0) {
        ...
        for (let e = 0; e < epochs; e++) {
            ...
            for (let start = 0; start < n; start += batchSize) {
                ...
                for (let r = 0; r < idx.length; r++) {
                    for (let c = 0; c < x.cols; c++)
                        bx.set(r, c, x.get(idx[r], c));
                    for (let c = 0; c < y.cols; c++)
                        by.set(r, c, y.get(idx[r], c));
                }
            ...
            losses.push(avg);
            this[TRAINING_TRACE].push({ kind: "success", epoch: e + 1, loss: avg, phase: TrainingPhase.EpochEnd });
            ...
            if (convergeThreshold > 0 && avg < convergeThreshold) {
                ...
                this[TRAINING_TRACE].push({ kind: "converged", epoch: e + 1, finalLoss: avg });
                if (verbose)
    ...
    predict(input) {
        ...
        for (let j = 0; j < out.cols; j++)
            arr.push(out.get(0, j));
        return arr;
    }
    /** 生成器：层迭代 */
    *iterLayers() { for (const l of this.layers)
        yield l; }
    /** 生成器：所有层的所有神经元迭代 */
    *iterNeurons() {
    ...
    summary() {
        const parts = [`Network[id=${this[MODEL_ID].substring(0, 8)}] state=${this._state} epoch=${this._epoch}`];
        for (const l of this.iterLayers())
            parts.push(`  ${l.layerType.padEnd(7)} ${l.activationType.padEnd(8)} in=${l.inputSize} out=${l.weightCols}`);
        return parts.join("\n");
    }
    serialize() {
        return JSON.stringify({ modelId: this[MODEL_ID], config: this._config, layers: this.layers.map((l) => l.snapshot()) });
    }
...
function formatResult(r) {
    switch (r.kind) {
        case "success": return `[epoch ${r.epoch}] loss=${r.loss.toFixed(6)} (${r.phase})`;
        case "error": return `[epoch ${r.epoch}] ERROR(${r.code}): ${r.message}`;
        case "converged": return `[epoch ${r.epoch}] converged finalLoss=${r.finalLoss.toFixed(6)}`;
    }
}
function summarizeTrace(trace) {
    let successes = 0, errors = 0, converged = 0;
    for (const r of trace) {
        if (isTrainSuccess(r))
            successes++;
        else if (isTrainError(r))
            errors++;
        else if (isTrainConverged(r))
            converged++;
    }
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < rest.length; i++) {
        ...
        switch (a) {
            ...
            case "--input": {
                const s = rest[++i] ?? "";
                s.split(",").map((n) => parseFloat(n)).forEach((n) => { if (!isNaN(n))
                    inputVec.push(n); });
                break;
            }
            default:
...
function runXorDemo(epochs, lr) {
    console.log("=== XOR 训练 ===");
    const baseConfig = {
        inputSize: 2, hidden: [4], outputSize: 1,
        hiddenActivation: ActivationType.Tanh, outputActivation: ActivationType.Sigmoid,
    ...
    console.log(`  trace: success=${t.successes} converged=${t.converged} errors=${t.errors}`);
}
function runClassifyDemo(epochs, lr, outFile) {
    console.log("=== 合成 3 分类训练 ===");
    const { inputs, targets, labels } = generateClassData();
    const net = new NeuralNetwork({
        inputSize: 2, hidden: [8, 6], outputSize: 3,
        hiddenActivation: ActivationType.Relu, outputActivation: ActivationType.Softmax,
        loss: "crossEntropy", learningRate: lr,
    });
    net.fit(inputs, targets, epochs, 16, true);
    let correct = 0;
    for (let i = 0; i < inputs.length; i++)
        if (argmax(net.predict(inputs[i])) === argmax(targets[i]))
            correct++;
...
function trainFromData(dataFile, epochs, lr, outFile) {
    ...
    const net = new NeuralNetwork({
        inputSize, hidden: raw.hidden ?? [8], outputSize,
        hiddenActivation: raw.hiddenActivation ?? ActivationType.Relu,
        outputActivation: raw.outputActivation ?? (outputSize === 1 ? ActivationType.Sigmoid : ActivationType.Softmax),
...
function predictModel(modelFile, input) {
...
function runSelfTest() {
    ...
    let msum = 0;
    for (const v of Matrix.fromArray([[1, 2], [3, 4]]))
        msum += v;
    console.log(`矩阵元素和 (迭代器): ${msum} (期望 10)`);
    // 演示 entries() 生成器
    let entryCount = 0;
    for (const _e of Matrix.fromArray([[1, 2], [3, 4]]).entries())
        entryCount++;
    ...
    console.log(`sigmoid(0) = ${s.get(0, 0).toFixed(4)} (期望 0.5) [via ${sigmoid.toString()}]`);
    const net = new NeuralNetwork({
        inputSize: 2, hidden: [4], outputSize: 1,
        hiddenActivation: ActivationType.Tanh, outputActivation: ActivationType.Sigmoid,
        loss: "mse", learningRate: 0.5,
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (got === want)
            ok++;
        console.log(`  XOR(${XOR_DATA.inputs[i][0]},${XOR_DATA.inputs[i][1]}) -> ${p[0].toFixed(3)} (期望 ${want})`);
    }
    console.log(`XOR 正确: ${ok}/4 ${ok === 4 ? "PASS" : "FAIL"}`);
    // 演示神经元生成器
    let neuronCount = 0;
    for (const _n of net.iterNeurons())
        neuronCount++;
    ...
    console.log(`训练 trace: success=${summary.successes} converged=${summary.converged} errors=${summary.errors}`);
    if (last)
        console.log(`  最后 trace: ${formatResult(last)}`);
    if (last && isTrainSuccess(last))
        console.log(`  (类型守卫确认 success: epoch=${last.epoch}, loss=${last.loss.toFixed(6)})`);
