    ...
    async handleRequest(req, res) {
        ...
        if (!found) {
            ...
            ctx.response.body = {
                ...
                availableRoutes: this.router.getRoutes().map((r) => `${r.method.padEnd(7)} ${r.path}`),
            };
        ...
        const handlerMiddleware = async (c, _next) => {
            handlerCalled = true;
...
class CrudController {
    constructor(resourceName, validationSchema) {
        this.resourceName = resourceName;
        this.validationSchema = validationSchema;
        this.store = new Map();
        this.idGenerator = new AutoIncrementId();
    }
    /** GET / — 列表（支持分页和过滤） */
    async index(ctx) {
        ...
        const pagination = { total, page: pageNum, pageSize: pageSizeNum, totalPages };
    ...
    async create(ctx) {
        ...
        const item = { id, ...ctx.request.body };
    ...
    async update(ctx) {
        ...
        const updated = { id, ...ctx.request.body };
...
function validateUser(data) {
    ...
    if (obj.role && !["admin", "editor", "viewer"].includes(obj.role)) {
...
// ============================================================
/** 首页 — API 概览 */
async function apiIndexHandler(ctx) {
    ctx.response.body = {
...
/** 管理后台（需要认证） */
async function adminDashboardHandler(ctx) {
    ctx.response.body = {
        message: "欢迎进入管理后台",
        user: ctx.state.user,
        stats: {
...
async function main() {
    ...
    // ── 用户资源 CRUD ──
    const userController = new CrudController("用户", validateUser);
    seedUsers(userController);
    app.resource("/api/users", userController);
    // ── 文章资源 CRUD ──
    const postController = new CrudController("文章", validatePost);
