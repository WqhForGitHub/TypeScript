...
function loadStore() {
    ...
    try {
        ...
        console.error("读取数据文件失败，将使用空数据。错误信息：", err.message);
...
function findTodoIndex(store, id) {
...
function cmdEdit(idStr, content) {
    const id = Number(idStr);
    if (!Number.isInteger(id) || id <= 0) {
        console.error("错误：ID 必须是正整数。");
        return;
    ...
    if (!content || content.trim().length === 0) {
        console.error("错误：请提供新的任务内容。用法：todo-cli edit <id> <新内容>");
        return;
    }
    const store = loadStore();
    const idx = findTodoIndex(store, id);
...
function cmdStats() {
    ...
    // 找出最早和最晚创建的任务
    const sortedByDate = [...store.todos].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    const earliest = sortedByDate[0];
    const latest = sortedByDate[sortedByDate.length - 1];
    // 找出最近完成的任务
    const completedTodos = store.todos.filter((t) => t.done && t.completedAt);
    const recentlyCompleted = completedTodos.length > 0
        ? [...completedTodos].sort((a, b) => new Date(b.completedAt).getTime() -
    ...
    if (recentlyCompleted) {
        console.log(`  最近完成：#${recentlyCompleted.id} ${recentlyCompleted.content}`);
