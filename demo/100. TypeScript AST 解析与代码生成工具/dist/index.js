...
function getKindLabel(kind) {
    return SYNTAX_KIND_LABELS[kind] || ts.SyntaxKind[kind] || `Unknown(${kind})`;
}
/** 缩进辅助 */
...
function parseToAST(code, fileName = "sample.ts") {
    return ts.createSourceFile(fileName, code, ts.ScriptTarget.Latest, true, // setParentNodes: 设置父节点引用
    ts.ScriptKind.TS);
}
// ============ 2. AST 可视化 ============
/**
...
function analyzeAST(sourceFile) {
    ...
    function visit(node) {
        ...
        if (ts.isClassDeclaration(node)) {
            ...
            node.members.forEach((member) => {
                if (ts.isConstructorDeclaration(member)) {
                    const params = member.parameters.map((p) => p.name.getText()).join(", ");
        ...
        if (ts.isInterfaceDeclaration(node)) {
            ...
            result.interfaces.push(`interface ${name} {\n${members.join("\n")}\n}`);
        }
        // 类型别名声明
 ...
 */
function renameIdentifier(sourceFile, oldName, newName) {
 ...
 */
function addDeprecatedTag(sourceFile) {
    const transform = (context) => {
        const visit = (node) => {
            if (ts.isFunctionDeclaration(node) && node.name) {
                // 使用 addSyntheticLeadingComment 添加 @deprecated 注释
                const newNode = ts.addSyntheticLeadingComment(node, ts.SyntaxKind.MultiLineCommentTrivia, "*\n * @deprecated 此函数已废弃，请使用新版本。\n ", true);
                return newNode;
            }
            return ts.visitEachChild(node, visit, context);
        };
 ...
 */
function addLoggingToFunctions(sourceFile) {
    const transform = (context) => {
        const visit = (node) => {
            if (ts.isFunctionDeclaration(node) && node.body && node.name) {
                const funcName = node.name.text;
                // 创建 console.log 调用语句
                const logStatement = ts.factory.createExpressionStatement(ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier("console"), ts.factory.createIdentifier("log")), undefined, [
                ...
                // 在函数体开头插入日志语句
                const newBody = ts.factory.createBlock([logStatement, ...originalStatements], true);
...
function buildASTProgramatically() {
    ...
    // ---- 构建 interface Person ----
    const personInterface = factory.createInterfaceDeclaration([factory.createModifier(ts.SyntaxKind.ExportKeyword)], factory.createIdentifier("Person"), undefined, // typeParameters
    undefined, // heritageClauses
    [
        factory.createPropertySignature(undefined, factory.createIdentifier("name"), undefined, factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword)),
        factory.createPropertySignature(undefined, factory.createIdentifier("age"), undefined, factory.createKeywordTypeNode(ts.SyntaxKind.NumberKeyword)),
    ]);
    // ---- 构建 createPerson 函数 ----
    const createPersonFunc = factory.createFunctionDeclaration([factory.createModifier(ts.SyntaxKind.ExportKeyword)], undefined, factory.createIdentifier("createPerson"), undefined, [
        factory.createParameterDeclaration(undefined, undefined, // dotDotDotToken
        factory.createIdentifier("name"), undefined, factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword)),
        factory.createParameterDeclaration(undefined, undefined, // dotDotDotToken
        factory.createIdentifier("age"), undefined, factory.createKeywordTypeNode(ts.SyntaxKind.NumberKeyword)),
    ], factory.createTypeReferenceNode(factory.createIdentifier("Person")), factory.createBlock([
        factory.createReturnStatement(factory.createObjectLiteralExpression([
            ...
            factory.createShorthandPropertyAssignment(factory.createIdentifier("age")),
        ], true)),
    ], true));
    // ---- 构建 greetPerson 函数 ----
    const greetPersonFunc = factory.createFunctionDeclaration([factory.createModifier(ts.SyntaxKind.ExportKeyword)], undefined, factory.createIdentifier("greetPerson"), undefined, [
        factory.createParameterDeclaration(undefined, undefined, // dotDotDotToken
        factory.createIdentifier("person"), undefined, factory.createTypeReferenceNode(factory.createIdentifier("Person"))),
    ], factory.createKeywordTypeNode(ts.SyntaxKind.StringKeyword), factory.createBlock([
        factory.createReturnStatement(factory.createTemplateExpression(factory.createTemplateHead("Hello, "), [
            factory.createTemplateSpan(factory.createPropertyAccessExpression(factory.createIdentifier("person"), factory.createIdentifier("name")), factory.createTemplateMiddle("! You are ")),
            factory.createTemplateSpan(factory.createPropertyAccessExpression(factory.createIdentifier("person"), factory.createIdentifier("age")), factory.createTemplateTail(" years old.")),
        ])),
    ], true));
    // ---- 构建 SourceFile ----
    const sourceFile = factory.createSourceFile([personInterface, createPersonFunc, greetPersonFunc], factory.createToken(ts.SyntaxKind.EndOfFileToken), ts.NodeFlags.None);
    return sourceFile;
}
// ============ 7. 综合示例：完整流水线 ============
/**
