                        ...
                        const typeColor = log.type === 'ERROR' ? '\x1b[31m' :
                            log.type === 'CALC' ? '\x1b[33m' :
                                log.type === 'TIMER' ? '\x1b[36m' :
                                    '\x1b[90m';
                        console.log(`  \x1b[90m${log.time}\x1b[0m ${typeColor}[${log.type}]\x1b[0m ${log.message}`);
                    }
                    console.log('─'.repeat(60));
                },
            });
                ...
                handler: () => {
                    ...
                    for (const p of allPlugins) {
                        const deps = p.meta.dependencies?.length
                            ...
                            : '';
                        console.log(`  \x1b[36m${p.meta.name}\x1b[0m v${p.meta.version} - ${p.meta.description}${deps}`);
                    }
                    console.log('─'.repeat(50));
                },
            });
