...
function displayCurrent(weather) {
    ...
    console.log();
    console.log(c(`  ${(0, api_1.getWeatherIcon)(current.weatherCode)}  ${locationStr} 当前天气`, color.bold, color.cyan));
    console.log(`  ${divider("═", 56)}`);
    console.log(`  ${c("天气：", color.bold)}${(0, api_1.getWeatherIcon)(current.weatherCode)} ${(0, api_1.getWeatherDescription)(current.weatherCode)}`);
    console.log(`  ${c("温度：", color.bold)}${c(`${current.temperature}°C`, color.yellow)}    ${c("体感：", color.bold)}${c(`${current.feelsLike}°C`, color.yellow)}`);
    console.log(`  ${c("湿度：", color.bold)}${current.humidity}%     ${c("气压：", color.bold)}${current.pressure.toFixed(0)} hPa`);
    console.log(`  ${c("风速：", color.bold)}${current.windSpeed} km/h  ${c("风向：", color.bold)}${windDirection(current.windDirection)}（${current.windDirection}°）`);
    console.log(`  ${c("降水：", color.bold)}${current.precipitation} mm   ${c("昼夜：", color.bold)}${current.isDay ? c("白天", color.yellow) : c("夜晚", color.blue)}`);
    console.log(`  ${divider("═", 56)}`);
    console.log();
}
function displayForecast(weather) {
    ...
    // 表头
    console.log(c("  日期         天气     最低  最高   降水    降水概率  风速", color.dim));
    console.log(`  ${divider("─", 72)}`);
    for (const day of daily) {
        ...
        const dateStr = isToday
            ...
            : formatDate(day.date);
        const icon = (0, api_1.getWeatherIcon)(day.weatherCode);
        const desc = (0, api_1.getWeatherDescription)(day.weatherCode);
        ...
        console.log(`  ${dateStr}  ${weatherStr} ${minTempStr} ${maxTempStr}  ${bar}  ${precipStr}  ${probStr}   ${windStr}`);
    }
    console.log(`  ${divider("═", 72)}`);
    console.log();
}
function displayHourly(weather) {
    ...
    for (const hour of displayHours) {
        ...
        const icon = (0, api_1.getWeatherIcon)(hour.weatherCode);
        const desc = (0, api_1.getWeatherDescription)(hour.weatherCode).padEnd(6);
        ...
        console.log(`  ${timeStr}  ${icon}${desc} ${tempStr}  湿度${humStr}  降水${precipStr}  风速${windStr}`);
    }
    console.log(`  ${divider("═", 64)}`);
    console.log();
}
...
async function queryAndDisplay(cityName, displayFn) {
    const loader = showLoading(`正在查询「${cityName}」的天气`);
    try {
        const location = await (0, api_1.geocode)(cityName);
        const weather = await (0, api_1.fetchWeather)(location);
        loader.stop();
        displayFn(weather);
    }
    catch (err) {
        loader.stop();
        console.error(c(`错误：${err.message}`, color.red));
    }
}
async function cmdNow(cityName) {
    if (!cityName || cityName.trim().length === 0) {
        console.error(c("错误：请提供城市名称。用法：weather-cli now <城市>", color.red));
        return;
    }
    await queryAndDisplay(cityName.trim(), displayCurrent);
}
async function cmdForecast(cityName) {
    if (!cityName || cityName.trim().length === 0) {
        console.error(c("错误：请提供城市名称。用法：weather-cli forecast <城市>", color.red));
        return;
    }
    await queryAndDisplay(cityName.trim(), displayForecast);
}
async function cmdHourly(cityName) {
    if (!cityName || cityName.trim().length === 0) {
        console.error(c("错误：请提供城市名称。用法：weather-cli hourly <城市>", color.red));
        return;
    }
    await queryAndDisplay(cityName.trim(), displayHourly);
}
async function cmdSearch(cityName) {
    if (!cityName || cityName.trim().length === 0) {
        console.error(c("错误：请提供城市名称。用法：weather-cli search <城市>", color.red));
        return;
    }
    const loader = showLoading(`正在搜索「${cityName}」`);
    try {
        const locations = await (0, api_1.geocodeMultiple)(cityName.trim());
        loader.stop();
        console.log();
        ...
        locations.forEach((loc, i) => {
            ...
            console.log(`  ${c(`[${i + 1}]`, color.green)} ${loc.name}${admin}，${loc.country}  (${loc.latitude.toFixed(2)}, ${loc.longitude.toFixed(2)})`);
        });
        console.log(`  ${divider("═", 50)}`);
        // 交互选择
        ...
        const answer = await new Promise((resolve) => {
            rl.question(c("  请输入编号查看天气（直接回车取消）：", color.yellow), resolve);
        });
        rl.close();
        ...
        if (idx >= 0 && idx < locations.length) {
            ...
            try {
                const weather = await (0, api_1.fetchWeather)(selected);
                loader2.stop();
...
main().catch((err) => {
    console.error(c(`致命错误：${err.message}`, color.red));
    process.exit(1);
});
