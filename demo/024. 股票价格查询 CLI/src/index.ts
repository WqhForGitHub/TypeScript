...
type SHCode = `sh${number}`;
type SZCode = `sz${number}`;
type AShareCode = SHCode | SZCode;
type UsTicker = `${Uppercase<string>}${string}`;
type StockCode = AShareCode | UsTicker;

// ============================================================
// 3. 映射类型 (Mapped types)
// ============================================================

type StockTypeLabel = { [K in StockType]: string };
const STOCK_TYPE_LABELS: StockTypeLabel = {
  [StockType.Stock]: "股票",
  [StockType.ETF]: "ETF基金",
  [StockType.Index]: "指数",
  [StockType.Bond]: "债券",
...
type TrendLabel = { readonly [K in TrendDirection]: string };
...
type MarketDataShape<T extends StockType> = T extends StockType.Index
  ? { constituents: number }
  : T extends StockType.Bond
    ? { yield: number }
    : { sharesOutstanding: number };

type UnwrapEnum<E> = E extends infer K ? K : never;

// ============================================================
// 5. 受约束泛型 (Generics with constraints)
// ============================================================

type MarketData<T extends StockType> = MarketDataShape<T> & {
...
// 示例: 泛型约束 + 条件类型组合的实例化
const SAMPLE_INDEX_DATA: MarketData<StockType.Index> = {
  symbol: "sh000001",
...
abstract class AbstractIndicator<C extends string> {
  constructor(public readonly config: Record<C, number>) {}
  abstract readonly name: string;
  abstract compute(prices: readonly number[]): number[];
}

// ============================================================
// 6. 元组 / 只读元组 (Tuples / readonly tuples)
// ============================================================
...
// ============================================================
// 7. 判别联合 (Discriminated unions)
// ============================================================

type StockEvent =
  | { kind: "PRICE_UP"; symbol: string; delta: number }
  | { kind: "PRICE_DOWN"; symbol: string; delta: number }
  | { kind: "VOLUME_SPIKE"; symbol: string; volume: number }
  | {
      kind: "TREND_CHANGE";
      symbol: string;
      from: TrendDirection;
...
type MarketSignal =
  | { type: "BUY"; strength: number; reason: string }
  | { type: "SELL"; strength: number; reason: string }
...
function isStockType(v: unknown): v is StockType {
  return (
    typeof v === "string" && (Object.values(StockType) as string[]).includes(v)
  );
}
function isMarketType(v: unknown): v is MarketType {
  return (
    typeof v === "string" && (Object.values(MarketType) as string[]).includes(v)
  );
}
function isStockCode(v: unknown): v is StockCode {
  if (typeof v !== "string") return false;
  return /^sh\d{6}$/i.test(v) || /^sz\d{6}$/i.test(v) || /^[A-Z]{1,6}$/.test(v);
}

// ============================================================
// 10. 接口 (optional/readonly/index signature) + 工具类型
// ============================================================
...
interface StockInfo {
  ...
  volume: number;
  marketCap: number;
  readonly history: DayQuote[];
  [key: string]: unknown; // index signature
}

// Utility types exercise
type StockSummary = Pick<
  StockInfo,
  "symbol" | "name" | "price" | "changePercent"
>;
type StockBrief = Omit<StockInfo, "history" | "marketCap">;
type MutableDayQuote = Partial<Record<keyof DayQuote, unknown>>;
type FrozenStock = Readonly<StockInfo>;
type IndicatorRegistry = Record<string, AbstractIndicator<string>>;
type GenerateStockReturn = ReturnType<typeof generateStock>;
type GenerateStockParams = Parameters<typeof generateStock>;
...
const TIME_RANGE_DAYS: Readonly<Record<TimeRange, number>> = {
  [TimeRange.Day]: 1,
  [TimeRange.Week]: 7,
  [TimeRange.Month]: 30,
...
const fmtVolume = (n: number): string => fmtMoney(n);

function colorChange(val: number, text: string): string {
  if (val > 0) return `\x1b[31m${text}\x1b[0m`; // 红涨 (中国习惯)
...
abstract class AbstractStockDataProvider {
  protected abstract readonly sourceName: string;
  abstract fetch(symbol: string): StockInfo;
  abstract fetchHistory(symbol: string, days: number): DayQuote[];
...
class MockStockProvider extends AbstractStockDataProvider {
  protected readonly sourceName = "MockDataProvider";
  fetch(symbol: string): StockInfo {
    return generateStock(symbol);
  }
  fetchHistory(symbol: string, days: number): DayQuote[] {
...
class MovingAverageIndicator extends AbstractIndicator<"period"> {
  readonly name = `MA(${this.config.period})`;
  compute(prices: readonly number[]): number[] {
...
class EMAIndicator extends AbstractIndicator<"period"> {
  readonly name = `EMA(${this.config.period})`;
  compute(prices: readonly number[]): number[] {
...
class RSIIndicator extends AbstractIndicator<"period"> {
  readonly name = `RSI(${this.config.period})`;
  compute(prices: readonly number[]): number[] {
    const { period } = this.config;
...
class MACDIndicator extends AbstractIndicator<"fast" | "slow" | "signal"> {
  readonly name = "MACD";
  compute(prices: readonly number[]): number[] {
    const emaFast = new EMAIndicator({ period: this.config.fast }).compute(
      prices,
    );
    const emaSlow = new EMAIndicator({ period: this.config.slow }).compute(
      prices,
    );
    const macd = prices.map((_, i) => emaFast[i] - emaSlow[i]);
    const signal = new EMAIndicator({ period: this.config.signal }).compute(
      macd,
    );
...
class BollingerIndicator extends AbstractIndicator<"period" | "stdDev"> {
  readonly name = "BOLL";
  compute(prices: readonly number[]): number[] {
...
function* iterateHistory(history: readonly DayQuote[]): Generator<DayQuote> {
  for (const q of history) yield q;
}
...
function* generateMockTicks(
  ...
  for (let i = 0; i < count; i++) {
    ...
    yield [now - (count - i) * 1000, round2(price)] as const;
  }
}
...
function detectTrend(history: readonly DayQuote[]): TrendDirection {
  if (history.length < 5) return TrendDirection.Sideways;
  ...
  if (pct > 0.02) return TrendDirection.Bullish;
  if (pct < -0.02) return TrendDirection.Bearish;
  return TrendDirection.Sideways;
}

function detectSupportResistance(history: readonly DayQuote[]): {
  support: number;
  resistance: number;
} {
...
function generateStock(symbol: string, days = 60): StockInfo {
  ...
  if (!isStockCode(sym)) throw new InvalidCodeError(sym);
  ...
  for (let i = days; i >= 1; i--) {
    ...
    const volume = Math.floor(
      (5_000_000 + dayRng() * 50_000_000) *
        (1 + (Math.abs(drift) / price) * 20),
    );
    history.push({
      date: d.toISOString().slice(0, 10),
      ohlc: [round2(open), round2(high), round2(low), round2(close)] as OHLC,
      volume,
    });
    price = close;
  }
  ...
  const now = new Date();
  const intradayRng = mulberry32(
    hashSeed(sym + now.toISOString().slice(0, 16)),
  );
  ...
  const high =
    Math.max(open, currentPrice) * (1 + intradayRng() * volatility * 0.4);
  const low =
    Math.min(open, currentPrice) * (1 - intradayRng() * volatility * 0.4);
  ...
  const market: MarketType = /^sh/i.test(sym)
    ? MarketType.SH
    : /^sz/i.test(sym)
      ? MarketType.SZ
      : MarketType.US;
  return {
    symbol: sym,
    name: STOCK_NAMES[sym as keyof typeof STOCK_NAMES] ?? `${sym} 公司`,
...
// ============================================================
// 20. 函数重载 (Function overloads)
// ============================================================

function query(symbol: string, range: TimeRange): StockInfo;
function query(
  symbol: string,
  range: TimeRange,
  format: OutputFormat,
...
class Portfolio {
  private entries: Map<string, PortfolioEntry> = new Map();
  private _label = "MyPortfolio";
  get label(): string {
    return this._label;
  }
  set label(v: string) {
    this._label = v || "MyPortfolio";
  }
  get size(): number {
  ...
  remove(symbol: string): boolean {
    return this.entries.delete(symbol);
  }
  *pnl(): Generator<{ symbol: string; pnl: number; pnlPercent: number }> {
    for (const [, entry] of this.entries) {
      const s = generateStock(entry.symbol);
      const pnl = round2((s.price - entry.costPrice) * entry.shares);
      const pnlPercent = round2(
        ((s.price - entry.costPrice) / entry.costPrice) * 100,
      );
...
function rankStocks(
  ...
  field: SortField = SortField.ChangePercent,
  ascending = false,
  limit = 5,
): StockInfo[] {
...
interface MarketStats {
  readonly gainers: StockInfo[];
  readonly losers: StockInfo[];
  readonly volumeLeaders: StockInfo[];
  readonly topMarketCap: StockInfo[];
}

function computeMarketStats(stocks: readonly StockInfo[]): MarketStats {
  return {
...
function generateSignal(s: StockInfo): MarketSignal {
  const rsiArr = new RSIIndicator({ period: 14 }).compute(
    s.history.map((h) => h.ohlc[3]),
  );
  const lastRsi = rsiArr[rsiArr.length - 1] ?? 50;
  if (s.changePercent > 3 && lastRsi < 70) {
    return {
      type: "BUY",
      strength: Math.min(1, s.changePercent / 10),
      reason: `涨幅 ${s.changePercent}% 且 RSI 未超买`,
    };
  }
  if (s.changePercent < -3 && lastRsi > 30) {
    return {
      type: "SELL",
      strength: Math.min(1, -s.changePercent / 10),
      reason: `跌幅 ${s.changePercent}% 且 RSI 未超卖`,
    };
  }
  return { type: "HOLD", reason: `RSI=${lastRsi.toFixed(1)}，无明显信号` };
}

function formatSignal(signal: MarketSignal): string {
  switch (signal.type) {
    case "BUY":
    case "SELL":
...
function buildEvent(s: StockInfo): StockEvent {
  if (s.change > 0)
...
function renderChart(closes: readonly number[], height = 10): string {
...
function formatStock(s: StockInfo, fmt: OutputFormat): string {
  switch (fmt) {
...
const provider: AbstractStockDataProvider = new MockStockProvider();
...
function cmdQuote(args: string[]): void {
  ...
  console.log(
    `当前价格: $${s.price.toFixed(2)}  ${colorChange(s.change, `${s.change >= 0 ? "+" : ""}${s.change.toFixed(2)} (${s.changePercent >= 0 ? "+" : ""}${s.changePercent.toFixed(2)}%)`)}`,
  );
  console.log(
    `昨收: $${s.prevClose.toFixed(2)}  开盘: $${s.open.toFixed(2)}  最高: $${s.high.toFixed(2)}  最低: $${s.low.toFixed(2)}`,
  );
  console.log(
    `成交量: ${fmtVolume(s.volume)}  市值: $${fmtMoney(s.marketCap)}`,
  );
  console.log(
    `趋势: ${TREND_LABELS[trend]}  支撑: $${sr.support.toFixed(2)}  阻力: $${sr.resistance.toFixed(2)}`,
  );
  console.log(`信号: ${formatSignal(signal)}`);
  // event narrowing
  const evtDesc =
    event.kind === "PRICE_UP" || event.kind === "PRICE_DOWN"
  ...
  const macd = new MACDIndicator({ fast: 12, slow: 26, signal: 9 }).compute(
    closes,
  );
  const boll = new BollingerIndicator({ period: 20, stdDev: 2 }).compute(
    closes,
  );
  const last = <T>(a: readonly T[]): T | undefined => a[a.length - 1];
  console.log(
    `MA5=${last(ma5)?.toFixed(2) ?? "-"}  MA20=${last(ma20)?.toFixed(2) ?? "-"}  RSI(14)=${last(rsi)?.toFixed(2) ?? "-"}  MACD=${last(macd)?.toFixed(2) ?? "-"}  BOLL上轨=${last(boll)?.toFixed(2) ?? "-"}`,
  );
  // 价格条
  const range = s.high - s.low || 1;
  const pos = Math.min(1, Math.max(0, (s.price - s.low) / range));
  const barLen = 30,
    filled = Math.round(pos * barLen);
  console.log(
    `价格区间: [${"█".repeat(filled)}${"░".repeat(barLen - filled)}]  ${s.low.toFixed(2)} - ${s.high.toFixed(2)}\n`,
  );
}

function cmdHistory(args: string[]): void {
  const symbol = args[0];
  if (!symbol) throw new StockError("请提供股票代码", "MISSING_ARG");
  let days = 30;
  ...
  console.log(
    renderTable(["日期", "开盘", "最高", "最低", "收盘", "成交量"], rows),
  );
  // 用生成器迭代收集收盘价
  const closes: number[] = [];
  for (const q of iterateHistory(recent)) closes.push(q.ohlc[3]);
  console.log(`\n收盘价走势 (高度 10):`);
  ...
  console.log(
    renderTable(
      ...
      stats.gainers.map((s) => [
        ...
        `$${s.price.toFixed(2)}`,
        `+${s.change.toFixed(2)}`,
        colorChange(s.changePercent, `+${s.changePercent.toFixed(2)}%`),
      ]),
    ),
  );
  console.log(`\n=== 跌幅榜 TOP 5 ===`);
...
function cmdWatch(args: string[]): void {
  ...
  console.log(
    `监控 ${symbol.toUpperCase()}，每 ${interval} 秒刷新一次 (Ctrl+C 退出)\n`,
  );
  const tick = (): void => {
    ...
    process.stdout.write(
      `[${time}] ${s.symbol} $${s.price.toFixed(2)} ${colorChange(s.change, `${s.change >= 0 ? "+" : ""}${s.change.toFixed(2)} (${s.changePercent >= 0 ? "+" : ""}${s.changePercent.toFixed(2)}%)`)}  昨收 $${s.prevClose.toFixed(2)}`,
    );
  };
  tick();
...
function cmdPortfolio(args: string[]): void {
