...
function safeParse(raw) {
    try {
        ...
        return [null, err instanceof SyntaxError ? err : new SyntaxError(String(err))];
    }
}
/**
 * 根据点分隔路径查询 JSON 对象
 ...
 */
function queryByPath(obj, queryPath) {
    if (queryPath.trim() === "") {
        return obj;
    }
    ...
    for (const token of tokens) {
        if (current === null || current === undefined) {
            throw new Error(`路径 "${queryPath}" 在 "${token}" 处遇到 null/undefined`);
        }
        if (Array.isArray(current)) {
            const index = Number(token);
            if (Number.isNaN(index) || index < 0 || index >= current.length) {
                throw new Error(`数组索引越界: [${token}] (数组长度: ${current.length})`);
            }
            ...
            throw new Error(`路径 "${queryPath}" 在 "${token}" 处遇到非对象类型: ${typeof current}`);
        }
    }
    return current;
}
 ...
 * 计算 JSON 对象的最大嵌套深度
 */
function calcDepth(obj, current = 0) {
...
function colorize(jsonStr) {
    return jsonStr.replace(/("(?:\\.|[^"\\])*")\s*:/g, `${COLOR.CYAN}$1${COLOR.RESET}:`) // 键名
        .replace(/:\s*("(?:\\.|[^"\\])*")/g, `: ${COLOR.GREEN}$1${COLOR.RESET}`) // 字符串值
        .replace(/:\s*(\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)/g, `: ${COLOR.YELLOW}$1${COLOR.RESET}`) // 数字
 ...
 * query: 用路径查询 JSON 值
 */
function cmdQuery(raw, queryPath) {
    const [obj, err] = safeParse(raw);
    if (err) {
        console.error(`${COLOR.RED}JSON 解析失败: ${err.message}${COLOR.RESET}`);
        process.exit(1);
    ...
    try {
        ...
        if (result === undefined) {
            console.log(`${COLOR.YELLOW}路径 "${queryPath}" 未找到任何值${COLOR.RESET}`);
            process.exit(1);
        }
...
function parseArgs(argv) {
    ...
    if (!VALID_COMMANDS.includes(command)) {
        throw new Error(`未知命令: "${command}" (可用命令: ${VALID_COMMANDS.join(", ")})`);
    }
    // 需要文件路径
    if (args.length < 2) {
        throw new Error(`命令 "${command}" 需要指定 JSON 文件路径 (或用 "-" 从 stdin 读取)`);
    }
    ...
    if (command === "query" && !queryPath) {
        throw new Error('query 命令需要指定查询路径, 例如: json-fmt query data.json "data.name"');
    }
