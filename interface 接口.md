...
注意，如果 `type` 命名定义的类型不是对象，interface 就无法继承。
...
上面示例中，接口 `Circle` 和 `Rectangle` 组成一个联合类型 `Circle | Rectangle` 。因此，这个联合类型的同名属性 `area`，也是一个联合类型。本例中的 `declare` 命名表示变量 `s` 的具体定义，由其他脚本文件给出，详见《declare 命名》一章。
...
综上所述，如果有复杂的类型运算，那么没有其他选择只能用 `type`；一般情况下，`interface` 灵活性比较高，便于扩充类型或自动合并，建议优先使用。
